        ...
        <div>
            {exists ? (
                <div className="max-w-screen-xl px-5">
                    <h1>
                        Join with code: <span className="text-red-500">{roomId}</span> or with this{" "}
                        <Link href={`/room/${roomId}`} className="text-blue-500">
                            link
                        </Link>
                    </h1>
                    <div>{messages && messages.docs.map((doc) => <Message key={doc.id} msg={doc.data()} />)}</div>
                    <form onSubmit={sendMessage} className="flex gap-2 w-full mt-10">
